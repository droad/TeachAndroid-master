...
public class ApiFacade {
    ...
    public void getDialogs (){
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                urlBuilder.append("messages.getDialogs");
                urlBuilder.append("?");
                ...
                try {
                    ...
                    Logger.log(TAG, "URL - dialogs " + urlString);
                    ...
                    while ((line = reader.readLine()) != null){
                        Logger.log(TAG, "LINE - dialogs " + line);
    ...
    public void getFriends (final ResponseListener<List<Friends>> listener){
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    urlBuilder.append("count").append("=").append(URLEncoder.encode("100", "UTF-8")).append("&");
                    urlBuilder.append("fields").append("=").append(URLEncoder.encode("photo_100", "UTF-8")).append("&");
                    urlBuilder.append("v").append("=").append(URLEncoder.encode("5.28", "UTF-8")).append("&");
                    urlBuilder.append("access_token").append("=").append(URLEncoder.encode(accessToken, "UTF-8"));
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                ...
                try {
                    String urlString = urlBuilder.toString();
                    URL url = new URL(urlString);
                    Logger.log(TAG, "URL - friends " + urlString);
                    connection = (HttpURLConnection) url.openConnection();
                    InputStream in = connection.getInputStream();

                    BufferedReader reader = new BufferedReader(new InputStreamReader(in));

                    ApiResponse<ResponseList<Friends>> apiResponse = new Gson().fromJson(reader, new TypeToken<ApiResponse<ResponseList<Friends>>>() {
                    }.getType());
                    if(apiResponse != null){
                        listener.onResponse(apiResponse.getResult().getItems());
                        listener.onError(apiResponse.getError());
                    ...
                    String line = null;
                    while ((line = reader.readLine()) != null){
                        Logger.log(TAG, "LINE - friends " + line);
